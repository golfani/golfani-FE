...
const FeedScrapModal = (props: IFeedScrapModalProps): JSX.Element => {
    ...
    const onSelectFeedItem = useCallback((id: number) => {
        const index = editFeed.indexOf(id);
        if (index === -1) {
            setEditFeed(editFeed => editFeed.concat(id));
        } else {
            setEditFeed(editFeed => editFeed.filter(item => item !== id));
        }
    }, []);
    ...
    const handleClickDeleteButton = useCallback(async () => {
        try {
            editFeed.map((scrapId) => {
                deleteScrapMutate.mutateAsync(scrapId);
            });
        } catch (e) {
            ...
            <div className={style.scrap_box} ref={feedScrapModalRef}>
                ...
                <div className={style.all_feed_scrap_box}>
                    {scrapFeed.map((scrapFeed) => (
                        <div className={style.img_box} key={scrapFeed.feed.id}
